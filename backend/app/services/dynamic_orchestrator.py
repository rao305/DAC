...
Guidelines:
- For SIMPLE queries (e.g., "define a common term"), you may compress to 1–3 steps:
  - analyst + creator
  - or creator + synthesizer
- For COMPLEX queries (system design, research-heavy, strategy, multi-part questions), prefer:
  - analyst → researcher → creator → critic → synthesizer
...
Schema:
{
  "pipeline_summary": "short natural-language summary (1–3 sentences) of the planned collaboration",
  "steps": [
    {
      "step_index": 1,
...
Details:
- `pipeline_summary`: 1–3 sentences maximum. Describe the overall plan at a high level.
...
- `estimated_importance` is a float 0.0–1.0 (how critical this step is to final quality).
- `model_rationale` explains in 1–3 sentences why this model is chosen.
