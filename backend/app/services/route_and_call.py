...
async def route_and_call(
    ...
    base_messages = build_prompt_for_model(thread_id, SYNTRA_SYSTEM_PROMPT)
    # Inject persona with intent awareness and provider specialization
    # Use the first provider in the fallback chain for specialized system prompt
    first_provider = fallback_chain[0][0].value if fallback_chain and len(fallback_chain) > 0 else None
    prompt_messages = inject_syntra_persona(base_messages, qa_mode=False, intent=intent, provider=first_provider)
