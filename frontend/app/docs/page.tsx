...
import { SDKCodeTabs } from '@/components/sdk-code-tabs'
...
export default function DocsPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Quickstart - SDK Code Tabs */}
      <section className="py-16 border-b border-border">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <Reveal>
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-foreground mb-4">Quickstart</h2>
              <p className="text-muted-foreground">
                Make your first API call in minutes with our simple SDK
              </p>
            </div>
          </Reveal>
              ...
              {filteredSections.map((section) => {
                ...
                return (
                  <motion.div key={section.id} variants={item}>
                    <Card
                      className="border-border bg-zinc-900/40 backdrop-blur-sm hover:bg-zinc-900/60 transition-all hover:-translate-y-1 cursor-pointer group h-full"
                    >
                      <Link href={`/docs/${section.id}`}>
                        <CardContent className="p-6">
                          <div className="flex items-start gap-4">
                            <motion.div 
                              className="w-12 h-12 rounded-lg bg-emerald-500/20 flex items-center justify-center flex-shrink-0 group-hover:bg-emerald-500/30 transition-colors"
            ...
            <Stagger>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
                <motion.div variants={item}>
                  <Card className="border-border bg-zinc-900/40 backdrop-blur-sm hover:bg-zinc-900/60 transition-all hover:-translate-y-1">
                    <CardContent className="p-6">
